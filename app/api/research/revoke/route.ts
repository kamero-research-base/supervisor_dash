...
export async function PUT(req: NextRequest): Promise<NextResponse> {
    ...
    try {
        let query = `UPDATE researches SET status = 'Pending', revoke_approval_reason = $2, approval_revoked_at = NOW(), revoker_supervisor_id = $3 WHERE hashed_id = $1 RETURNING id`;
   
        // Update research status back to pending with revoke reason
